...
export async function centralRoutes(fastify: FastifyInstance, options: FastifyPluginOptions) {
  ...
  fastify.register(async (privateRoutes) => {
    ...
    privateRoutes.post("/centrais", controller.create.bind(controller));
    privateRoutes.get("/centrais", controller.list.bind(controller));
    privateRoutes.delete("/centrais", controller.delete.bind(controller));
    privateRoutes.put("/centrais", controller.update.bind(controller));
  });
}
